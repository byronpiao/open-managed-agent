...
/** opencode: harness_sync_events + session/load replay (local testSyncPersistence parity). */
export async function verifyCloudOpencodeSync(agentId, envId) {
  ...
  const body = await client.promptSse(sessionId, "Reply with exactly: OK", 301);
  const text = extractSseText(body);
  if (!text.trim()) {
    throw new Error(`opencode cloud sync: empty LLM reply: ${body.slice(0, 300)}`);
  }

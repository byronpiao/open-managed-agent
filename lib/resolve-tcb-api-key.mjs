 ...
 * Sets process.env.CLOUDBASE_APIKEY when needed — OAK reads that name; users only configure CAM.
 */
...
/** Set process.env.CLOUDBASE_APIKEY when unset, using CAM-derived gateway token. */
export async function ensureTcbApiKeyInProcess(options = {}) {
  if (process.env.CLOUDBASE_APIKEY?.trim()) return process.env.CLOUDBASE_APIKEY.trim();
  const key = await resolveTcbApiKey(options);
  process.env.CLOUDBASE_APIKEY = key;
  return key;
}

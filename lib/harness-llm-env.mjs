 ...
 * AGENT_MODEL overlays agent.yaml only when AGENT_ENV_OVERRIDES=1
 * (debug / test). Product config: write model in agent.yaml.
 */
...
export function applyZenLlmEnv(target = process.env) {
  ...
  target.AGENT_ENV_OVERRIDES = "1";
}

/** CloudBase platform gateway (hy3 etc.) — no custom LLM env */

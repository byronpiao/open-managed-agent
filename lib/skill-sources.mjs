...
export function parseSkillSource(source) {
  ...
  if (raw.startsWith("git:")) return { kind: "git", payload: raw.slice(4) };
  if (raw.startsWith("skillhub:")) return { kind: "skillhub", payload: raw.slice(9) };
  if (raw.startsWith("skills.sh:")) return { kind: "skillssh", payload: raw.slice(10) };
  if (raw.startsWith("file:")) return { kind: "local", payload: raw.slice(5) };
  throw new Error(
    `Skill source must use a protocol prefix (file:, git:, skillhub:, skills.sh:): ${raw}`,
  );
}

export function deployNameFromSkillDir(skillDir) {
...
export async function resolveSkillArtifacts(skill, ctx) {
  ...
  if (!source) throw new Error("Skill entry requires source");

  const { kind, payload } = parseSkillSource(source);
  managedTrace("skill-source.resolve", { source, kind, payloadPreview: payload.slice(0, 120) });
  ...
  if (kind === "local") {
    ...
    return [{ destName: deployNameFromSkillDir(skillDir), skillDir }];
  }

  if (kind === "git") {
    const { cloneUrl, branch, subpath } = normalizeGitUrl(payload);
    managedTrace("skill-source.git_clone", { source, cloneUrl, branch, subpath });
    const repoDir = doClone(cloneUrl, branch, tempParent);

    if (subpath) {
      const skillDir = findSkillDir(repoDir, { subpath });
      return [{ destName: deployNameFromSkillDir(skillDir), skillDir, tempRoot: repoDir }];
    }
    ...
    try {
      ...
      if (scanned.length === 1) {
        return [{
          destName: deployNameFromSkillDir(scanned[0].skillDir),
          skillDir: scanned[0].skillDir,
          tempRoot: repoDir,
        }];
      }
  ...
  if (kind === "skillssh") {
    ...
    managedTrace("skill-source.skillssh_clone", { source, owner, skillId, cloneUrl });
    const repoDir = doClone(cloneUrl, undefined, tempParent);
    const skillDir = findSkillDir(repoDir, { skillId });
    return [{ destName: skillId, skillDir, tempRoot: repoDir }];
  }

  if (kind === "skillhub") {
    const slug = parseSkillhubSlug(payload);
    managedTrace("skill-source.skillhub_fetch", { source, slug });
    const { skillDir, tempRoot } = await fetchSkillhubDir(slug, fetchImpl);
    return [{ destName: slug, skillDir, tempRoot }];
  }

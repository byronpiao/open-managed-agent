...
export async function uploadCloudRunSourcePackage(envId, serviceName, sourceDir) {
  process.stdout.write(dim("  uploading source package... "));
  const { UploadUrl, UploadHeaders, PackageName, PackageVersion } =
    await describeBuildService(envId, serviceName);
  const zip = await zipDir(sourceDir);
  await uploadZipBuffer({ uploadUrl: UploadUrl, headers: UploadHeaders, buffer: zip });
  console.log(`OK (${(zip.length / 1024).toFixed(1)} KiB)`);
  return { packageName: PackageName, packageVersion: PackageVersion };
}
...
export async function buildCloudRunEnvParam({ envId, configB64, config = null, agentId }) {
...
// agentConfig.metadata matches the stamp we wrote at deploy time.
export function agentConfigMatchesDeployStamp(
  agentConfig,
  { deployedAt, skillHashes } = {},
) {
  const meta = agentConfig?.metadata;
  if (!meta || meta.__deployedAt !== deployedAt) return false;
  if (!skillHashes || Object.keys(skillHashes).length === 0) return true;
  const live = meta.__skillHashes;
  if (!live || typeof live !== "object") return false;
  for (const [name, hash] of Object.entries(skillHashes)) {
    if (live[name] !== hash) return false;
  }
  return true;
}

export async function waitForConfigLive({
  agentUrl,
  expectedDeployedAt,
  expectedSkillHashes,
  maxWaitMs = 5 * 60 * 1000,
  ...
  while (Date.now() - startedAt < maxWaitMs) {
    try {
      ...
      if (
        agentConfigMatchesDeployStamp(result?.agentConfig, {
          deployedAt: expectedDeployedAt,
      ...
      // transient — agent might be in mid-roll or cold-starting, retry
    }
    await new Promise((r) => setTimeout(r, 5000));
  }
  return false;

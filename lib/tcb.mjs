...
// ── Code download helpers ───────────────────────────────────────────────────

/**
 * Download SCF function code to local directory.
 * Uses: tcb fn code download <functionName> <dest> -e <envId>
 */
export function downloadScfCode(envId, functionName, destDir) {
  // Ensure dest dir exists
  if (!existsSync(destDir)) {
    mkdirSync(destDir, { recursive: true });
  }

  console.log(`Downloading SCF function code: ${functionName}...`);
  const raw = runTcb([
    "fn", "code", "download",
    functionName,
    destDir,
  ...
  if (files.length === 0) {
    throw new Error(`Downloaded code directory is empty: ${destDir}`);
  }

  console.log(`Code downloaded to ${destDir} (${files.length} files)`);
  return destDir;
}

/**
 * Download CloudRun service code to local directory.
 * Uses: tcb cloudrun download -s <serviceName> --targetPath <dest> -e <envId>
 */
export function downloadCloudRunCode(envId, serviceName, destDir) {
  // Ensure dest dir exists
  if (!existsSync(destDir)) {
    mkdirSync(destDir, { recursive: true });
  }

  console.log(`Downloading CloudRun service code: ${serviceName}...`);
  const raw = runTcb([
    "cloudrun", "download",
    "-s", serviceName,
    "--targetPath", destDir,

...
export async function confirmManagedSkillDeploy({
  ...
  const skillNames = Object.keys(expectedHashes ?? {});
...
export function skillsChanged(currentSkills, newSkills, { forceSync = false } = {}) {
  ...
  const norm = (arr) =>
    JSON.stringify(
      (arr ?? []).map((s) => s.source).filter(Boolean).sort(),
    );
...
export async function syncSkillsInDir(desiredSkills, deployedSkillsDir, opts = {}) {
  ...
  const desiredSources = new Set(
    (desiredSkills ?? []).map((s) => s.source?.trim()).filter(Boolean),
  );
  ...
  /** @type {Array<{ skill: { source: string }, artifacts: import('./skill-sources.mjs').resolveSkillArtifacts extends Function ? Awaited<ReturnType<...>> : never }>} */
  const resolved = [];
  const desiredDestNames = new Set();
  /** @type {Map<string, string>} */
  const destNameSources = new Map();

  for (const skill of desiredSkills ?? []) {
    const artifacts = await resolveSkillArtifacts(skill, ctx);
    const source = skill.source?.trim() ?? "";
    for (const art of artifacts) {
      const prev = destNameSources.get(art.destName);
      if (prev && prev !== source) {
        console.warn(
          yellow(
            `    ⚠️  Skill name '${art.destName}' overwritten: ${prev} → ${source}`,
          ),
        );
      }
      destNameSources.set(art.destName, source);
      desiredDestNames.add(art.destName);
    }
    resolved.push({ skill, artifacts });
  }

  for (const bundleKey of Object.keys(manifest.bundles ?? {})) {
    if (!desiredSources.has(bundleKey)) {
      const removed = removeBundleSkills(deployedSkillsDir, manifest, bundleKey);
      result.removed.push(...removed);
    }
  }

  for (const name of listSkillNamesInDir(deployedSkillsDir)) {
    if (!desiredDestNames.has(name)) {
  ...
  for (const { skill, artifacts } of resolved) {
    const source = skill.source?.trim() ?? "";
    console.log(`  Processing ${source}...`);
    const sourceKind = source.startsWith("git:")
      ? "git"
      : source.startsWith("skillhub:")
        ? "skillhub"
        : source.startsWith("skills.sh:")
          ...
          : source.startsWith("file:")
            ? "file"
            : "unknown";
    skillSyncPhase("install_skill_start", { source, sourceKind });
    const tempsToClean = new Set();
    ...
    try {
      ...
      skillSyncLog()?.error(err, { phase: "install_skill", source });
      throw err;
    } finally {
      for (const t of tempsToClean) {
        try {

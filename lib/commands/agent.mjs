...
async function handleAgentUpdate(options) {
  ...
  // SCF path — mirror agent:create: bundle code + full env replace via tcb agent update.
  ...
  const scfUpdateEnv = await buildScfDeployEnvMap(envId, merged, configBase64, { agentId });
  const envStr = scfEnvMapToCliArg(scfUpdateEnv);
  const memorySize = merged.runtime === "harness" ? "512" : "256";
  const code = resolveCodePath(options.code);

  // Bundle node_modules locally so the SCF function has deps available (same as agent:create).
  const deployDir = resolve("/tmp", `magent-scf-${agentId}-${Date.now()}`);
  let actualCode = code;
  try {
    execSync(`rm -rf "${deployDir}" && mkdir -p "${deployDir}"`, { encoding: "utf-8" });
    const filesToCopy = ["dist", "package.json", "package-lock.json", "scf_bootstrap", "vendor"];

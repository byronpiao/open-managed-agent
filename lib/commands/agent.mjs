...
import { scfAgentFullUpdateArgs, SCF_DEPLOY_IGNORE } from "../scf-bundle.mjs";
...
async function bundleScfDeployDir(sourceCode, deployLabel, config, { strict = false, configFile = null } = {}) {
  ...
  try {
    ...
    // Include `src/` + `tsconfig.json` so the deployed code package supports
    // secondary development (download → edit src → npm run build → redeploy)
    // without needing a separate source drop.
    const filesToCopy = ["dist", "src", "package.json", "package-lock.json", "scf_bootstrap", "tsconfig.json", "vendor"];
    ...
    dropBundleLockfile(deployDir);
    // Cloud-side dependency install (--install-dep): never ship a locally
    // installed node_modules — a macOS install can diverge from the linux cloud
    // build. The cloud builds deps from package.json. The 256MB claude platform
    // binary is NOT installed by the cloud build (optional dep skipped); the
    // runtime downloads it at cold start via scf_bootstrap.
...
/**
 * Drop the bundle's package-lock.json before install.
 *
 * The workspace install records local packages (open-managed-agent-runtime-shared
 * etc.) as workspace links. A standalone /tmp bundle can't follow those links,
 * so npm install would misbehave. Deleting the lock lets npm re-resolve: the
 * published open-managed-agent-runtime-shared is now fetched from the registry.
 */
function dropBundleLockfile(deployDir) {
  const lockPath = resolve(deployDir, "package-lock.json");
  if (!existsSync(lockPath)) return;
  try {
    rmSync(lockPath);
  } catch (err) {
    console.log(yellow(`  Warning: failed to drop bundle lockfile: ${err.message?.split("\n")[0]}`));
  }
}
...
function scfCodeDeployTcbArgs(actualCode) {
  return [
    "--code", actualCode,
  ...
  try {
    applyScfAgentUpdateViaTcb(
      [
        ...
        "--env", envStr,
        "-e", envId,
        ...scfCodeDeployTcbArgs(actualCode),
        "--timeout", "7200",

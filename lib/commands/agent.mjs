...
const SCF_DEPLOY_IGNORE = ".git,node_modules,.DS_Store,.deploy,.deploy-cloudrun,logs";
...
function bundleScfDeployDir(sourceCode, deployLabel, { strict = false } = {}) {
  ...
  try {
    ...
    if (existsSync(resolve(sourceCode, "agent.yaml"))) filesToCopy.push("agent.yaml");
    if (existsSync(resolve(sourceCode, "skills"))) filesToCopy.push("skills");
  ...
  return { deployDir, actualCode, sourceCode };
}
...
async function updateScfHarnessAgent(agentId, envId, config, codeOpt) {
  ...
  const { deployDir, actualCode } = bundleScfDeployDir(sourceCode, agentId, { strict: true });
  try {
    applyScfAgentUpdateViaTcb(
      [
...
export async function handleAgentCreate(options) {
  ...
  const { deployDir, actualCode, sourceCode } = bundleScfDeployDir(code, name, {
...
export function registerAgentCommands(program) {
  ...
  program.command("agent:update")
    ...
    .option("--code <path>", "[harness SCF only] Code directory (default: auto-resolve)")

...
export async function handleAgentCreate(options) {
  ...
  const code    = options.code    ?? "./packages/agent-runtime";
  const scfRuntime =
    options.scfRuntime ??
...
async function handleScfImageCreate(options) {
  ...
  const namespace = options.namespace ?? process.env.CCR_NAMESPACE;
  if (!namespace) {
...
export function registerAgentCommands(program) {
  program.command("agent:create")
    ...
    .option("--type <type>", "Compute backend (scf|scf-image|tcbr)", "tcbr")
  ...
  program.command("scf-image:create")
    ...
    .option("--namespace <ns>", "CCR namespace (or set CCR_NAMESPACE)")

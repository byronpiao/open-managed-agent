...
const {
  ...
  BASELINE_IMAGE, COS_BASELINE_URL, TCR_NAMESPACE,
} = process.env;
...
if (!TCR_NAMESPACE) {
  console.error('[pull] TCR_NAMESPACE not set'); process.exit(1);
}
...
//   A. 有密码 → 直接 docker login
//   B. 无密码 → CreateUserPersonal 首次开通；ErrUserExist → exit 引导路径 A
//
// 平台注入：API_SECRET_ID/KEY/TOKEN, CLOUDBASE_SERVICE_NAME, CLOUDBASE_VERSION_NAME, CLOUDBASE_ENV_ID
// 本地显式传入：TCR_REGISTRY, TCR_NAMESPACE, CCR_USERNAME, CCR_OWNER_UIN, SECRET_CCR_PASSWORD (via Secrets[])
...
const {
  TCR_REGISTRY, CLOUDBASE_SERVICE_NAME, CLOUDBASE_VERSION_NAME,
  CLOUDBASE_ENV_ID = 'unknown', TCR_NAMESPACE,
  CCR_USERNAME, CCR_OWNER_UIN,
  SECRET_CCR_PASSWORD, CCR_PASSWORD,
  API_SECRET_ID = '', API_SECRET_KEY = '', API_TOKEN = '',
} = process.env;

if (!TCR_REGISTRY || !CLOUDBASE_SERVICE_NAME || !CLOUDBASE_VERSION_NAME) {
  console.error('[login] missing required platform vars'); process.exit(1);
}
if (!TCR_NAMESPACE) {
  console.error('[login] TCR_NAMESPACE not set'); process.exit(1);
}
...
async function tcrApi(action, payload) {
  ...
  const cr = \`\${date}/tcr/tc3_request\`;
  const hcs = sha256hex(\`POST\\n/\\n\\n\${ch}\\n\${sh}\\n\${hp}\`);
  const sts = \`TC3-HMAC-SHA256\\n\${ts}\\n\${cr}\\n\${hcs}\`;
  const sd = hmacSha256Str(\`TC3\${API_SECRET_KEY}\`, date);
  const sv = hmacSha256Hex(sd, 'tcr');
  const ss = hmacSha256Hex(sv, 'tc3_request');
  const sig = hmacSha256Hex(ss, sts);
  const auth = \`TC3-HMAC-SHA256 Credential=\${API_SECRET_ID}/\${cr}, SignedHeaders=\${sh}, Signature=\${sig}\`;
  const resp = await fetch(\`https://\${host}\`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json; charset=utf-8',
      Host: host, 'X-TC-Action': action, 'X-TC-Version': '2019-09-24',
      'X-TC-Region': 'ap-guangzhou', // personal TCR is only available in ap-guangzhou
...
function tcrErrCode(r) { return r.Response?.Error?.Code || ''; }

function dockerLogin(user, pass) {
  const r = spawnSync('docker', ['login', '-u', user, '--password-stdin', TCR_REGISTRY], {
    input: pass, encoding: 'utf-8', stdio: ['pipe', 'inherit', 'inherit'],
  });
  if (r.status !== 0) { console.error('[login] docker login failed'); process.exit(1); }
}

async function ensureNamespace() {
  const r = await tcrApi('CreateNamespacePersonal', { Namespace: TCR_NAMESPACE });
  const code = tcrErrCode(r);
  if (!code || code.includes('NamespaceExist')) {
    console.log(\`[login] namespace ready: \${TCR_NAMESPACE}\`);
    return;
  }
  if (code.includes('NamespaceMaxLimit')) {
    console.error('\\n  命名空间数量已达上限');
    console.error('  1) 控制台删除旧命名空间: https://console.cloud.tencent.com/tcr');
    console.error('  2) 用 --namespace <已有命名空间> 复用');
    process.exit(1);
  }
  console.error(\`[login] CreateNamespacePersonal failed: \${code}\`);
  process.exit(1);
}
...
if (suppliedPass) {
  ...
  if (!suppliedUser) {
    console.error('[login] path A: CCR_USERNAME not set');
    process.exit(1);
  }
  user = suppliedUser;
  pass = suppliedPass;
  ...
  const reg = await tcrApi('CreateUserPersonal', { Password: autoPw });
  const errCode = tcrErrCode(reg);
  if (errCode.includes('ErrUserExist')) {
    console.log('');
    console.log('=========================================');
    console.log('  TCR 个人版已开通，请提供访问凭证');
    console.log('  已开通请用 --password <密码> 或 .env TCR_PASSWORD');
    console.log('=========================================');
    process.exit(1);
  }
  if (errCode) {

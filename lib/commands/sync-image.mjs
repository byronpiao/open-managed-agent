...
if (TCR_INSTANCE_ID) {
  ...
  if (!username || !password) {
    ...
    console.error('[login] 请确认企业版实例 ID、地域和 CAM 权限是否正确');
    console.error('[login] 参考（企业版快速入门）：https://cloud.tencent.com/document/product/1141/39287');
    process.exit(1);
  }
...
// ── personal ────────────────────────────────────────────
  ...
  if (suppliedPass) {
    ...
    if (errCode.includes('ErrUserExist')) {
      console.log('');
      console.log('=========================================');
      console.log('  TCR 个人版已开通，但未提供访问凭证密码');
      console.log('');
      console.log('  解决方法（任选其一）：');
      console.log('    1. .env 文件：TCR_PASSWORD=<控制台访问凭证密码>');
      console.log('    2. CLI 参数：magent sync-image -p <密码>');
      console.log('');
      console.log('  若忘记密码：TCR 控制台 → 个人版 → 更多 → 重置登录密码');
      console.log('    https://console.cloud.tencent.com/tcr');
      console.log('');
      console.log('  参考（个人版快速入门）：');
      console.log('    https://cloud.tencent.com/document/product/1141/63910');
      console.log('=========================================');
      process.exit(1);
    }
    if (errCode) {
    ...
    console.log('');
    console.log('=========================================');
    console.log('  首次开通 TCR 个人版（CreateUserPersonal）成功');
    console.log(\`  ENV_ID: \${CLOUDBASE_ENV_ID}\`);
    console.log('');
    console.log('  本次密码已自动生成并用于 docker login。');
    console.log('  ⚠  请立即从 CloudApp 构建日志中找到此密码（搜索 autoPw / path B），');
    console.log('  然后在项目根目录的 .env 中填写：');
    console.log('    TCR_PASSWORD=<此次自动生成的密码>');
    console.log('  否则下次同步将再次触发此流程（零配置只对首次有效）。');
    console.log('=========================================');
    console.log('');
  }
 ...
 */
async function ensurePersonalTcr({ loginUin: uinIn = "", ownerUin: ownerUinIn = "", password: pwIn = "", namespace: nsIn = "" } = {}) {
  // Reuse pre-fetched identity if available to avoid double API call
  const identity = (uinIn && ownerUinIn) ? { uin: uinIn, ownerUin: ownerUinIn } : await getCallerIdentity();
  ...
  if (!password) {
    ...
    if (!isExist || !mainIsExist) {
      ...
      if (cuCode.includes("ErrUserExist")) {
        ...
        throw new Error(
          ...
          `请切换到主账号，完成 TCR 个人版初始化后再切回子账号：\n` +
          `  1. tcb login（主账号）\n` +
          `  2. magent sync-image（首次完成初始化）\n` +
          `  3. tcb login（切回子账号）\n` +
          `  4. magent sync-image -p <密码>\n` +
          `参考（个人版快速入门）：https://cloud.tencent.com/document/product/1141/63910`,
        );
      } else if (cuCode) {
    ...
    if (!password) {
      const isSubaccount = loginUin !== ownerUin;
      const hint = isSubaccount
        ? `子账号已注册 TCR，请通过以下方式之一提供访问凭证密码：\n` +
          `  CLI 参数：magent sync-image -p <密码>\n` +
          `  .env 文件：TCR_PASSWORD=<密码>\n` +
          `密码可在 TCR 控制台「访问凭证」页面查看：https://console.cloud.tencent.com/tcr\n` +
          `参考（个人版快速入门）：https://cloud.tencent.com/document/product/1141/63910`
        : `主账号已注册 TCR，请通过以下方式之一提供访问凭证密码：\n` +
          `  CLI 参数：magent sync-image -p <密码>\n` +
          `  .env 文件：TCR_PASSWORD=<密码>\n` +
          `若忘记密码，可在 TCR 控制台「个人版」→「更多」→「重置登录密码」重置：https://console.cloud.tencent.com/tcr`;
      throw new Error(`缺少 TCR 访问凭证密码。\n${hint}`);
    }
  } else {
...
async function uploadZip(envId, serviceName, scripts) {
  ...
  if (!uploadUrl) throw new Error("DescribeCloudAppCosInfo 未返回 UploadUrl，请检查环境权限");
  if (!ts) throw new Error("DescribeCloudAppCosInfo 未返回 UnixTimestamp");
...
export function registerSyncImageCommand(program) {
  program
    ...
    .option("-p, --password <password>", "访问凭证密码（个人版；企业版走 STS 不需要）")
    .option("-i, --images <list>", "镜像: sandbox,tcbr,scf,all（默认 all）")
    .option("-e, --env-id <id>", "TCB 环境 ID（默认自动检测）")
    .option("-n, --namespace <ns>", "命名空间（个人版默认 tcb-<主账号UIN>；企业版必填）")
    .option("--mode <mode>", "TCR 模式: personal | enterprise（默认 personal）")
    .option("--tcr-id <id>", "企业版 TCR 实例 ID（enterprise 模式必填）")
    .option("--tcr-region <region>", "企业版 TCR 实例地域（enterprise 模式必填，如 ap-guangzhou）")
    .option("--endpoint <url>", "仓库地址（个人版默认 ccr.ccs.tencentyun.com；企业版必填，格式 <实例名>.tencentcloudcr.com）")
...
async function handleSyncImage(options) {
  ...
  if (mode !== "personal" && mode !== "enterprise") {
    console.error(red(`未知 TCR 模式: ${mode}。可用: personal | enterprise`));
    process.exit(1);
  }
  const isEnterprise = mode === "enterprise";

  // 企业版前置校验
  const tcrInstanceId = options.tcrId || process.env.TCR_INSTANCE_ID || "";
  const tcrRegion = options.tcrRegion || process.env.TCR_REGION || "";
  // 企业版 endpoint = <实例名>.tencentcloudcr.com
  // 注意：实例名（用户自定义，决定域名）≠ 实例 ID（系统生成，tcr-xxx，用于 API 调用）
  // 两者不同，无法互推，必须由用户显式提供 --endpoint
  const enterpriseEndpoint = options.endpoint || process.env.TCR_ENDPOINT || "";
  if (isEnterprise) {
    if (!tcrInstanceId) {
      console.error(red("enterprise 模式需要 --tcr-id（或 .env TCR_INSTANCE_ID=tcr-xxxxxxxx）"));
      process.exit(1);
    }
    if (!tcrRegion) {
      console.error(red("enterprise 模式需要 --tcr-region（如 ap-guangzhou / ap-shanghai / ap-beijing）"));
      process.exit(1);
    }
    if (!enterpriseEndpoint) {
      console.error(red(
        "enterprise 模式需要 --endpoint（企业版仓库域名，如 mycompany.tencentcloudcr.com）\n" +
        "提示：域名前缀 = 购买实例时填写的「实例名」，与实例 ID（tcr-xxx）不同\n" +
        "可在 TCR 控制台「实例管理」页面查看「实例域名」字段"
      ));
      process.exit(1);
    }
  }
  ...
  // ── registry endpoint ──────────────────────────────────
  // 个人版: ccr.ccs.tencentyun.com（固定，全局唯一入口）
  // 企业版: <实例名>.tencentcloudcr.com（由用户购买时的实例名决定，必须显式提供）
  const endpoint = isEnterprise ? enterpriseEndpoint : (options.endpoint || "ccr.ccs.tencentyun.com");

  const baselineTag = options.baselineTag || "latest";
  const results = [];

  // ── 个人版：本地 ensure（对齐 cloudappserver EnsureNamespace）────
  let loginUin = "", ownerUin = "", namespace = "", resolvedPassword = "";

  if (!isEnterprise) {
    let password = options.password || process.env.TCR_PASSWORD || "";

    if (!password && process.stdin.isTTY) {
      const hasCreds = await question("已有 TCR 个人版访问凭证？[Y/n]", "n");
    ...
    try {
      ensured = await ensurePersonalTcr({
        loginUin: options.uin || process.env.TCR_UIN || preflightIdentity?.uin || "",
    ...
    ({ loginUin, ownerUin, namespace } = ensured);
    resolvedPassword = ensured.password;
    ...
    // ── 企业版：STS CreateInstanceToken，本地无需 ensure ──
    namespace = options.namespace || process.env.TCR_NAMESPACE || "";
    if (!namespace) {
      console.error(red("enterprise 模式需要 --namespace（企业版 TCR 命名空间）"));
      process.exit(1);
    }
    console.log(`  目标: ${endpoint}/${namespace}`);
    console.log(`  实例: ${tcrInstanceId}  地域: ${tcrRegion}`);
    console.log(`  登录: STS CreateInstanceToken（平台自动）`);
    console.log("");
  }
    ...
    try {
      ...
      if (!isEnterprise) {
        envVars = [
          ...
          // TCR_REGION: personal 固定 ap-guangzhou（CNB 默认值），此处显式传入供参考
      ...
      if (!ver) {
        ...
        if (errMsg.includes("未开放")) throw new Error("当前环境暂未开通 CloudApp 能力，请联系服务提供方开启");
        throw new Error(`创建构建失败: ${errMsg}`);
      }
  ...
  if (results.length > 0) {
    ...
    // 个人版：提示保存凭证
    if (!isEnterprise && resolvedPassword && !options.noSave && process.stdin.isTTY) {
      const save = await question("\n保存配置到 .env？下次可直接 magent sync-image [Y/n]", "Y");
      if (save.toUpperCase() === "Y") {
        const envPath = resolve(process.cwd(), ".env");
        let content = existsSync(envPath) ? readFileSync(envPath, "utf-8") : "";
        // 只保存密码，不保存 UIN：UIN 与登录账号绑定，固化到 .env 会在切换账号时注入错误的 CCR_USERNAME

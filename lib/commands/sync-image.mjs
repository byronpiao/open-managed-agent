...
export function registerSyncImageCommand(program) {
  program
    ...
    .option("--endpoint <url>", "仓库地址（个人版默认 ccr.ccs.tencentyun.com；企业版默认 <tcr-id>.tencentcloudcr.com）")
...
async function handleSyncImage(options) {
  ...
  // ── registry endpoint ──────────────────────────────────
  // 个人版: ccr.ccs.tencentyun.com（固定）
  // 企业版: <实例ID>.tencentcloudcr.com（各实例独立域名）
  const endpoint = options.endpoint
    || (isEnterprise ? `${tcrInstanceId}.tencentcloudcr.com` : "ccr.ccs.tencentyun.com");

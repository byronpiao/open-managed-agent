...
export function registerSyncImageCommand(program) {
  program
    ...
    .option("--images <list>", "镜像: sandbox,oma-tcbr,oma-scf,all")
    .option("--env-id <id>", "TCB 环境 ID")
    .option("--endpoint <url>", "仓库地址")
...
async function handleSyncImage(options) {
  ...
  const namespace = options.namespace || `tcb-${uin || "user"}`;
